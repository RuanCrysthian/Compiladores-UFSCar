...
public class LaVisitor extends laBaseVisitor<Void> {
            ...
            if (currentScope.exists(variableIdentifier.toString())) {
                ...
                switch (variableType) {
                    ...
                    default:
                        ...
                        if (currentScope.exists(variableType) && currentScope.check(variableType).identifierType == SymbolTable.TypeLAIdentifier.TIPO) {
                            if (currentScope.exists(variableIdentifier.toString())) {
                                LaUtils.addSemanticError(ctxIdentVariable.IDENT(0).getSymbol(),
                                        "identificador " + variableIdentifier + " ja declarado anteriormente\n");
                            }
                        ...
                        // If the type is not declared, add a semantic error indicating that the type is undeclared.
    ...
    public Void visitCmd(laParser.CmdContext ctx) {
        // Logic for the rule "cmd" (command), i.e., handling actions when a command is found during code analysis.
        if (ctx.cmdLeia() != null) {
            var currentScope = nestedScopes.getCurrentScope();

            // Iterate over the identifiers in the command and perform semantic type verification.
        ...
        if (ctx.cmdAtribuicao() != null) {
            ...
            if (!LaUtils.verifyType(leftValue, rightValue) && !assignment[0].contains("^")) {
                // Add a semantic error if the assignment is not compatible with the identifier present in the assignment.
